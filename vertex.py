...
def vertex(ID):
    ...
    while not done_flag:
        ...
        if data.decode() == '1':
            ...
            parent_color = communicate_colors(ID, color, output_file_lock, children, children_sockets,
            ...
            if len(color) > 3:
                ...
                if shift_down:
                    ...
                    color = parent_color if parent else str((color + 1) % 3)
                    shift_down = False
                else:
                    if dec(color) == x:
    ...
    parent_color = communicate_colors(ID, color, output_file_lock, children, children_sockets,
                                      parent, parent_connection[0])
    if parent_color == color or int(color) > 2:
        print(f'ID:{ID}, color: {color}, parent_color: {parent_color}\n')
...
def communicate_colors(ID, color, output_file_lock, children, children_sockets, parent=None, parent_socket=None):
...
def min_non_conflicting_color(children_color, parent_color):
    for i in range(3):
        if dec(children_color) != i and dec(parent_color) != i:
            return bin(i)[2:].zfill(2)


def dec(binary_string: str):
    return int(binary_string, 2)


if __name__ == '__main__':
    print(min_non_conflicting_color(2, 0))
    # vertex('00101')
